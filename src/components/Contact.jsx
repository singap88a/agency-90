...
const Contact = ({ isPage = false }) => {
  ...
  return (
    <section id="contact" className={cn(
      "relative overflow-hidden font-['Cairo'] bg-white",
      isPage ? "py-24" : "py-32"
    )}>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="text-center mb-20 space-y-6">
            ...
            >
              ...
              <span className="text-xs font-black uppercase tracking-widest">{t('contact.formTitle', 'تواصل معنا')}</span>
            </motion.div>
            ...
            <motion.h2 
              ...
              className="text-4xl lg:text-5xl font-black text-brand-secondary tracking-tight"
            >
              {i18n.language === 'ar' ? (
          ...
          <div className="grid lg:grid-cols-[0.9fr_1.1fr] gap-12 lg:gap-16 items-start">
            ...
            >
              {/* البطاقة البيضاء */}
              <div className="bg-white rounded-[2.5rem] p-8 lg:p-10 shadow-2xl border border-slate-100 overflow-hidden relative h-full flex flex-col">
                {/* خلفية ناعمة جداً */}
                ...
                <div className="relative z-10 flex-1 flex flex-col">
                  {/* عنوان البطاقة مع خط احترافي - يبدأ تقيل وينتهي خفيف */}
                  <div className="mb-8">
                    <h3 className="text-2xl font-black mb-4 tracking-tight flex items-center gap-2 text-brand-secondary">
                      <MessagesSquare className="w-6 h-6 text-brand-primary" />
                      {t('contact.infoTitle')}
                    </h3>
                    {/* خط احترافي - Gradient يبدأ تقيل (100%) وينتهي خفيف (0%) */}
                    <div className="w-24 h-1.5 bg-gradient-to-r from-brand-primary via-brand-primary to-transparent rounded-full"></div>
                  </div>

                  {/* طرق التواصل - كل واحدة تحت الثانية (عمودي) */}
                  <div className="space-y-3 flex-1">
                    {contactMethods.map((item) => (
                      ...
                      >
                        <div className="flex items-center gap-4 p-4 rounded-2xl bg-slate-50/80 hover:bg-slate-100 border border-slate-100 hover:border-brand-primary/20 transition-all duration-300">
                          <div className="shrink-0">
                            <div className="w-12 h-12 rounded-xl bg-brand-primary/10 flex items-center justify-center text-brand-primary group-hover:bg-brand-primary group-hover:text-white transition-all duration-300 group-hover:scale-110 shadow-sm">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-lg font-bold text-brand-secondary group-hover:text-brand-primary transition-colors truncate">
                              {item.value}
                            </p>
                          </div>
            ...
            {/* 📝 Form Section - كل حقل تحت الثاني */}
            <motion.div
              initial={{ opacity: 0, scale: 0.95 }}
              whileInView={{ opacity: 1, scale: 1 }}
              viewport={{ once: true }}
              className="bg-white p-8 lg:p-10 rounded-[2.5rem] border border-slate-100 shadow-2xl relative overflow-hidden h-full"
            >
              ...
              <form onSubmit={handleSubmit} className="space-y-8 relative z-10">
                ...
                <div className="space-y-3">
                  ...
                  <div className="relative group/field">
                    ...
                    <input
                      ...
                      className={cn(
                        "w-full bg-slate-50/50 rounded-xl py-4 focus:outline-none focus:ring-2 focus:ring-brand-primary/20 border border-slate-100 focus:border-brand-primary/50 focus:bg-white transition-all font-bold text-brand-secondary",
                ...
                <div className="space-y-3">
                  ...
                  <div className="relative group/field">
                    ...
                    <select
                      ...
                      onChange={handleChange}
                      className={cn(
                        "w-full bg-slate-50/50 rounded-xl py-4 appearance-none focus:outline-none focus:ring-2 focus:ring-brand-primary/20 border border-slate-100 focus:border-brand-primary/50 focus:bg-white transition-all font-bold text-brand-secondary cursor-pointer",
                ...
                {/* الرسالة - لوحدها في سطر */}
                <div className="space-y-3">
                  <div className={cn(
                    "flex items-center gap-2 mb-1",
                    "justify-start"
                  )}>
                    <PenLine className="w-3.5 h-3.5 text-brand-primary" />
                    <label className="text-xs font-black text-slate-500 uppercase tracking-wider">
                  ...
                  <div className="relative group/field">
                    ...
                    <textarea
                      ...
                      rows="4"
                      className={cn(
                        "w-full bg-slate-50/50 rounded-xl py-4 focus:outline-none focus:ring-2 focus:ring-brand-primary/20 border border-slate-100 focus:border-brand-primary/50 focus:bg-white transition-all font-bold text-brand-secondary resize-none",
                        isRtl ? "pr-12 pl-5 text-right" : "pl-12 pr-5 text-left"
                      )}
                ...
                <button
                  type="submit"
                  className="w-full h-20 bg-brand-primary hover:bg-brand-secondary text-white rounded-2xl font-black text-xl transition-all duration-300 transform active:scale-95 flex items-center justify-center gap-4 shadow-xl shadow-brand-primary/20 group overflow-hidden relative"
                >
                  <span className="relative z-10">{t('contact.send', 'إرسال الرسالة')}</span>
                  <Send className={cn(
                    "relative z-10 w-6 h-6 transition-all duration-500 group-hover:translate-x-1 group-hover:-translate-y-1",
          ...
          <motion.div
            ...
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            className="mt-24 rounded-[2.5rem] overflow-hidden border border-slate-100 shadow-2xl shadow-slate-200/50 relative group"
          >
